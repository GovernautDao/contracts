...
contract TestGovernautGovernance is Test {
  ...
  function setUp() public {
    ...
    governautGovernance.initialize(token, address(identityManager));
    vm.stopPrank();
  }
  ...
  function testVerifiedProposeFunctionCreatesProposal() public {
    ...
    uint256 proposalId = governautGovernance.propose(
      targets,
      values,
      calldatas,
      description
  ...
  function testProposalExecution() public DelegateVotingPower {
    ...
    assertTrue(
      isApproved,
      "Proposer should be approved after successful proposal execution."
    );
  }
  ...
  function testVotingPeriodConstraints() public DelegateVotingPower {
    ...
    vm.warp(
      block.timestamp +
        governautGovernance.votingDelay() +
        governautGovernance.votingPeriod() +
        1
    );
