...
contract DeployContracts is Script {
    ...
    function run() external {
        ...
        HelperConfig.Config memory config = helperConfig.getOpSepoliaConfig(); // Or use the appropriate network
            // config
        identityManager = new IdentityManager(config._WorldcoinRouterAddress, config._appid, config._actionId);
        console.log("IdentityManager deployed at:", address(identityManager));

        // Deploy GovernautGovernance
        governance = new GovernautGovernance(IVotes(address(governanceToken)), address(identityManager));
        console.log("GovernautGovernance deployed at:", address(governance));

        // Deploy Funding
        funding = new Funding(vm.addr(deployerPrivateKey), address(governance), address(governanceToken));
